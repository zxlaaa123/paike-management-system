...
const semesterFormDefaults: SemesterForm = {
  name: '',
  schoolYear: '',
  term: '',
  startDate: '',
...
function toSemesterForm(data: SemesterForm): SemesterForm {
  return {
    name: data.name,
...
const {
  ...
  handleSubmit,
  handleDelete,
} = useCrudForm<Semester, SemesterForm>({
  fetchList: (q) =>
    getSemesterList({
  ...
  createItem: (data) => createSemester(toSemesterForm(data)),
  updateItem: (id, data) => updateSemester(id, toSemesterForm(data)),
  deleteItem: deleteSemester,
  searchDefaults: { keyword: '', status: '' },
  formDefaults: semesterFormDefaults,
  entityName: '学期',
})
...
async function handleDeleteSemester(row: Semester) {
  if (row.isCurrent === 1) {
