...
export function getUnavailableTimeList(params: {
  ...
  return request.get<ApiResponse<PageResult<TeacherUnavailableTime>>>('/teacher-unavailable-times', { params }).then((r) => r.data.data)
}

export function createUnavailableTime(data: UnavailableTimeForm) {
  return request.post<ApiResponse<TeacherUnavailableTime>>('/teacher-unavailable-times', data).then((r) => r.data.data)
}

export function updateUnavailableTime(id: number, data: UnavailableTimeForm) {
  return request.put<ApiResponse<TeacherUnavailableTime>>(`/teacher-unavailable-times/${id}`, data).then((r) => r.data.data)
}
