...
-- 全部包装为存储过程并 IF NOT EXISTS 判断，重复执行无副作用。

DELIMITER $$

DROP PROCEDURE IF EXISTS add_semester_current_unique$$
CREATE PROCEDURE add_semester_current_unique()
BEGIN
    -- 1) 数据 dedupe：如果历史上有 >=2 行 is_current=1，只保留 updated_at 最大的那一行；
    --    否则 ALTER 添加唯一约束会因数据冲突直接失败。
    IF (SELECT COUNT(*) FROM semester WHERE is_current = 1) > 1 THEN
        UPDATE semester
        SET is_current = 0,
            updated_at = NOW()
        WHERE is_current = 1
    ...
    IF NOT EXISTS (
        ...
        ALTER TABLE semester
            ADD COLUMN current_marker TINYINT GENERATED ALWAYS AS
                (CASE WHEN is_current = 1 THEN 1 ELSE NULL END) VIRTUAL,
            ADD UNIQUE KEY uk_semester_current_marker (current_marker);
    END IF;
END$$

DELIMITER ;

CALL add_semester_current_unique();
DROP PROCEDURE add_semester_current_unique;
