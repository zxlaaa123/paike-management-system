...
public class ScheduleLockController {
    ...
    @PostMapping("/lock")
    public Result<ScheduleLockActionVo> lock(@Valid @RequestBody ScheduleLockRequest request) {
        ScheduleLockActionVo result = scheduleLockService.lock(request);
        return Result.success("课程已锁定", result);
    }

    @PostMapping("/unlock")
    public Result<ScheduleLockActionVo> unlock(@Valid @RequestBody ScheduleLockRequest request) {
